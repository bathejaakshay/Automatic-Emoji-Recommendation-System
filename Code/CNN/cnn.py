"""
A Convolution Neural Network to recommend the emojis for a
...
class cnn:
    ...
    def __init__(self,stop_words,lemmatizer):
        """
        Initializing the cnn class with stop_words and lemmatizer as its instance variables
...
class cnn_run:
    ...
    declared in the cnn class and training and tesing the model
    ...
    def run(self):
        ...
        cnn_model = cnn(stop_words,lemmatizer)
        ...
        data, all_emojis = cnn_model.data_preprocessing(data, all_emojis)
        ...
        emoji_matrix = cnn_model.generate_emoji_matrix()
        ...
        cnn_model.set_emo(emo_list,emoji_set)

        data_df['label'] = data_df['label'].apply(cnn_model.new_label)
        data_df = data_df.dropna()

        
        data_df['label'] = data_df['label'].apply(cnn_model.clean)

        vect,train_sent,vocab_size,train_sent_X,train_label_y,mlb, max_len = cnn_model.vectorizer(data_df)
        ...
        model = cnn_model.make_model(vocab_size,top,max_len)
        ...
        for i in range(al):
            ...
            y,y_h = cnn_model.find_sem(y_test[i], y_hat[i], new_emoji_matrix, emoji_set, top)
            true_group.append(y)
    ...
    def recommend_em(self,vect,model,mlb,new_emoji_matrix,emoji_set,top,max_len):
        ...
        :parameter model: trained CNN model
